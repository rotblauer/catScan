...
- **3D Moments (volumetric video)** — a third capture mode records colored depth point clouds at 15 fps (10 / 30 / 60 second cap, your choice). Play them back from any angle with orbit + scrub controls, then compose a "spatial replay" video in a live preview: drag to frame the shot, **flick to set the spin** (the flick's direction becomes the rotation axis, its strength the speed), grab to lock a tripod shot, pick the loop count, and export to Photos — footage from angles the phone never stood at.
